...
def test(model, test_loader, loss_criterion, device, hook):
    ...
    for inputs, labels in test_loader:
        ...
        running_corrects += torch.sum(preds == labels.data)     # calculate the running corrects
...
def net():
    ...
    for param in model.parameters():
        param.requires_grad = False # freeze the convolution layer model
        nfeatures = model.fc.in_features
    model.fc = nn.Sequential(
                   nn.Linear(nfeatures, 512),
                   nn.ReLU(inplace = True),
                   nn.Linear(512, 256), # adding fullt connected NN layers to the output of the pretrained model
                   nn.ReLU(inplace=True),
                   nn.Linear(256, 133)) # output should be 133 since there are 133 classes of dog breeds
...
def main(args): 
    ...
    torch.save(model.state_dict(), os.path.join(args.model_dir, "model.pth")) # saving the trained model to S3
